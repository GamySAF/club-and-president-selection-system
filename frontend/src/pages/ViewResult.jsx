...
const ViewResults = () => {
  ...
  const [error, setError] = useState(null); // Added this to prevent 'setError' errors
  ...
  const fetchResults = async () => {
    ...
    try {
      ...
      // ✅ THE DATA FIX: Handle if the backend returns just an array
      let rawCandidates = [];
      let totalStudents = 0;
      let totalVoted = 0;

      if (Array.isArray(json)) {
        // If it's just an array, calculate votes from it
        rawCandidates = json;
        totalVoted = json.reduce((sum, c) => sum + (c.votes || 0), 0);
        totalStudents = totalVoted; // Fallback since backend didn't send count
      } else {
        // If it's an object, use the keys
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-10">
          ...
          <button 
            ...
            className="p-3 bg-white border border-slate-200 rounded-2xl hover:bg-slate-50 shadow-sm"
          >
            ...
            return (
              <div 
                ...
                className={`relative bg-white border-2 p-6 rounded-[2.5rem] transition-all duration-500 ${
                  index === 0 ? "border-indigo-500 ring-4 ring-indigo-50 shadow-lg" : "border-slate-100 shadow-sm"
                }`}
              >
                {index === 0 && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-indigo-600 text-white px-4 py-1 rounded-full text-xs font-black uppercase flex items-center gap-1 shadow-md">
                ...
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="text-xs font-bold text-indigo-600 uppercase mt-1">{c.party}</p>
                  </div>
                ...
                <div className="mt-6">
                  ...
                  <div className="h-3.5 w-full bg-slate-100 rounded-full overflow-hidden border border-slate-200/50">
                    <div 
                      className={`h-full rounded-full transition-all duration-1000 ${index === 0 ? "bg-indigo-600" : "bg-slate-400"}`}
